...
public:
  WeightedAutomatonModel()
      : reductionMethods({std::make_shared<KieferSchuetzenbergerReduction>()}),
        conversionMethods({}) {}
  ...
  validate_model_instance(std::string &str) override {
    ...
    if (line.starts_with("input=dense")) {
      ...
      throw std::invalid_argument(
          "The input specification must be either using sparse or dense format "
          "and declare "
          "this in the first line as 'sparse' or 'dense'!");
    }
  }

  static std::shared_ptr<RepresentationInterface>
  validate_model_instance_dense(std::string &str) {
    auto line = get_next_line(str, true);
    ...
    MatSpDPtr muX;
    muX = std::make_shared<MatSpD>(states, states);
    do {
      if (!line.starts_with("(")) {
        throw std::invalid_argument(
            "A new transition matrix should start here or the final vector!");
      }
      for (int i = 0; i < states; i++) {
    ...
    return std::make_shared<WeightedAutomaton>(states, characters, alpha, mu,
                                               eta);
  }

  static std::shared_ptr<RepresentationInterface>
  validate_model_instance_sparse(std::string &str) {
    size_t a = 0;
    long b = 0, c = 0;
    double d = 0.0;
    ...
    do {
      ...
      // fetch next line
      line = get_next_line(str, false);
    } while (!line.starts_with("mu") && !line.empty());
  ...
  [[nodiscard]] std::string summarize_reduction(
    ...
    auto WA = std::static_pointer_cast<WeightedAutomaton>(A);
    auto minWA = std::static_pointer_cast<WeightedAutomaton>(minA);
