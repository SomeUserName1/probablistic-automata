...
public:
  ...
  [[nodiscard]] std::shared_ptr<ModelInterface> select_model(
      const std::vector<std::shared_ptr<ModelInterface>> &) const override;

  [[nodiscard]] uint
  select_reduction_method(std::shared_ptr<ModelInterface> &) const override;

  [[nodiscard]] std::shared_ptr<ConversionMethodInterface>
  select_conversion_method(std::shared_ptr<ModelInterface> &) const override;

  [[nodiscard]] IOMethod select_io_method(bool) const override;

  [[nodiscard]] std::string file_input() const override;

  [[nodiscard]] std::string
  stdin_input(std::shared_ptr<ModelInterface> &) const override;
